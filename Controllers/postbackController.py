...
def postback_processer(GET_EVENT, data):
    ...
    if data['action'][0]=='confirm_learn':
        ...
        if temp_statement:
            ...
            GET_EVENT["replyList"] = [{"type": "text", "msg": "好哦已新增～"+GET_EVENT["postfix"]}]
    ##放棄詞條內容
    if data['action'][0]=='cancel_learn':
        temp_statement = get_temp_statement(data['id'][0])
        if temp_statement:
            delete_temp_statement(data['id'][0])
            GET_EVENT["replyList"] = [{"type": "text", "msg": "已放棄新增～"+GET_EVENT["postfix"]}]
    #----------------------------------------#
    ##詞條有幫助
    if data['action'][0]=='valid_response':
        ...
        if temp_statement:
            ...
            GET_EVENT["replyList"] = [{"type": "text", "msg": "感謝您的回饋～"+GET_EVENT["postfix"]}]
    ##詞條無幫助
